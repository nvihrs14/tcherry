context("tcherry_order_increase")
library(tcherry)
...
tcherry_ord_inc <- tcherry_order_increase(ChowLiu_cliques, data,
                                          smooth = 0.1)
...
test_that("error messages work", {
  expect_error(tcherry_order_increase(ChowLiu_cliques, data_numeric,
                                      smooth = 0.001),
               "Some columns are not characters or factors.")
  expect_error(tcherry_order_increase(ChowLiu_cliques, vec,
                                      smooth = 0.001),
               "data must be a data frame or a matrix.")
  expect_error(tcherry_order_increase(vec, data, smooth = 0.1),
               "Cliques must be given in a list, each entry containing a vector
         with the names of the variables in the clique.")
  expect_error(tcherry_order_increase(ChowLiu_cliques[-1], data,
                                      smooth = 0.1),
               "The column names of data must be the same as the variable
         names in tch_cliq. All variables in data must be in at least
         one clique.")
  expect_error(tcherry_order_increase(cliques_error, data,
                                      smooth = 0.1),
               "The column names of data must be the same as the variable
         names in tch_cliq. All variables in data must be in at least
         one clique.")
  expect_error(tcherry_order_increase(cliques_error2, data,
                                      smooth = 0.1),
               "tch_cliq should be the cliques of a k'th order t-cherry tree.
         Therefore they should all have the same length k.")
  expect_error(tcherry_order_increase(cliques_small, data[,1:2],
                                      smooth = 0.1),
               "It takes at least k plus 1 variables to fit a k plus 1'th order t-cherry tree.")
  expect_error(tcherry_order_increase(cliques_not_triang, data,
                                      smooth = 0.1),
  ...
  expect_error(tcherry_order_increase(cliques_wrong_nedges, data[, 1:5],
                                      smooth = 0.1),
