...
test_that("an error occur when zero probabilities", {
  expect_error(MI2(x1, y1), "Some probabilities are zero and therefore MI cannot be calculated.
...
test_that("an error occur when zero probabilities", {
  expect_error(MI3(x1, y1, z1), "Some probabilities are zero and therefore MI cannot be calculated.
         Consider using the smooth argument.")
})
